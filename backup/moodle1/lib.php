 ...
 * @copyright  Work based on : 2011 Andrew Davis <andrew@moodle.com>
 ...
 */
class moodle1_mod_signoff_handler extends moodle1_resource_successor_handler {
    ...
    public function process_legacy_resource(array $data, array $raw = null) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_signoff', 'intro');
        $url['intro'] = moodle1_converter::migrate_referenced_files($url['intro'], $this->fileman);

        // write signoff.xml
        $this->open_xml_writer("activities/signoff_{$moduleid}/signoff.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'signoff', 'contextid' => $contextid));
        $this->write_xml('url', $url, array('/signoff/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/signoff_{$moduleid}/inforef.xml");
